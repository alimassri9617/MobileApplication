...
export const useLostAndFound = () => {
  ...
  const createItem = async (itemData) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        Toast.show({ type: 'error', text1: data.message || 'Failed to post item' });
      }
    } catch (err) {
      Toast.show({ type: 'error', text1: 'Error creating item' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteItem = async (id) => {
    ...
    try {
      ...
      Toast.show({ type: 'error', text1: 'Error deleting item' });
    } finally {
      setLoading(false);
    }
  };
